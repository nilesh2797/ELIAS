...
python train.py configs/${dataset}/elias-1-def-final.yaml --num_epochs 5
...
python train.py configs/${dataset}/elias-1-def-final.yaml
# Generate row-wise sparse approximate adjacency graph (A) between clusters and labels based on trained ELIAS-1 model
python elias_utils.py gen_approx_A Results/${project}/${dataset}/elias-1-def-final/config.yaml
...
python train.py configs/${dataset}/elias-2-def-final.yaml
